...
class TextMatcher:
    ...
    def get_match_tags(cls, matched_texts_left: List[Paragraph],
        ...
        for text_left, text_right in zip(matched_texts_left, matched_texts_right):
            all_opcodes.append(cls.get_edit_operations(
    ...
    def compute_optimal_matches(cls, texts_left: List[Paragraph],
                                texts_right: List[Paragraph],
                                ratio_threshold) -> List[Tuple[int, Paragraph, int,
                                                               Paragraph, float]]:
        """
    ...
    def update_texts_combined(self):
        ...
        positions_left, matched_texts_left, positions_right, matched_texts_right, _ = zip(
            *optimal_matches)
        match_tags = self.get_match_tags(
            matched_texts_left, matched_texts_right)  # type: ignore
        ...
        self.texts_left = [
            text for text in self.texts_left if len(text.text) > 0]
        self.texts_right = [
            text for text in self.texts_right if len(text.text) > 0]
